use `local-portal`;

drop table if exists zz_roles_permissions;
drop table if exists zz_users_roles;
drop table if exists z_user_confirms;
drop table if exists z_user_locks;

drop table if exists databasechangelog;
drop table if exists databasechangeloglock;

drop table if exists audit_events;
drop table if exists e_audit_event_types;
drop table if exists e_operation_types;
drop table if exists e_permissions;

drop table if exists zz_festivals_albums;
drop table if exists festivals;
drop table if exists places;

drop table if exists zz_albums_files;
drop table if exists albums;
drop table if exists storage_files;

drop table if exists ip_bans;
drop table if exists persistent_logins;
drop table if exists statistic_logs;
drop table if exists roles;
drop table if exists users;
